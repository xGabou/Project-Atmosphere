...
- Added the PA-native volumetric cloud pipeline (`clouds.client.render.volumetric`): cloud cells are splatted into a camera-following 2D weather map (`cloud_weather_splat`), then one global fullscreen raymarch (`cloud_atmosphere_volume`) renders all clouds as a continuous density field with baked Perlin-Worley/Worley 3D noise, Beer-powder extinction, dual-lobe Henyey-Greenstein phase, multi-scatter octaves, and sunset/sunrise-aware sun and ambient colors from `VolumetricCloudLighting`.
- Added `CloudNoiseTextureManager`: asynchronous worker-thread bake of 128^3 Perlin-Worley base noise, 64^3 Worley detail noise, and 128^2 void-and-cluster blue noise, uploaded once as GL 3D/2D textures.
- Added temporal reprojection with blue-noise jitter, disocclusion rejection, ping-pong history targets (`VolumetricCloudRenderTargets`), LOW/MEDIUM/HIGH/ULTRA quality profiles (`VolumetricQualityProfile`), and a GPU frame-time governor (`CloudFrameTimeGovernor`) that scales raymarch steps to stay in budget.
- Added the CPU-authoritative `CloudCell` simulation (`CloudCellSimulationManager`): cells form/mature/dissipate near players following regional cloud cover and humidity, drift with altitude-sheared forecast wind, merge with overlap+streak hysteresis, split from GPU lobe evidence or oversized low-energy sheets, and are classified after the fact from measured shape (`CloudCellClassifier`: humilis/mediocris/congestus/cumulonimbus/stratocumulus/stratus/cirriform).
- Added cell network sync: `SyncCloudCellsPacket` (full, on join/dimension change), `CloudCellDeltaPacket` (dirty cells + removals every 10 ticks), and client-side Hermite interpolation/extrapolation in `ClientCloudCellCache` (protocol bumped to 8).
- Added the GL 4.3 compute analytics pass (`CloudCellAnalyticsPass`): measures per-cell integrated coverage, footprint ratio, centroid drift, column top, best-neighbor bridge overlap, and split score from the weather map with strictly fence-gated buffer readback (no GPU stall); digests are shipped to the server via `CloudCellAnalyticsPacket` as advisory merge/split evidence only.
- Added GPU-resident cloud ground shadows: `cloud_shadow_map` integrates weather-map optical depth into a 512^2 transmittance texture, `cloud_shadow_apply` darkens the scene through depth reconstruction as a multiplicative post-process; the texture id plus an analytically derived CPU 64x64 grid are published through `CloudShadowMapAccess`/`CloudShadowSnapshot` for shader packs and gameplay.
- Added interior cloud coupling: `CameraCloudDensityTracker` + `VolumetricCloudWhiteoutFogHandler` drive Minecraft fog distance/color from analytic in-cloud density, and the public `CloudDensityQuery` API exposes analytic cloud density on both sides (plane turbulence, icing, AI, audio consumers).
- Added tornado readiness hooks: cells carry `rotation`/`funnelStrength` properties, the volume shader has two analytic funnel SDF slots blended into the cloud density (`FunnelUniforms`), rotation spins up in energetic sheared cumulonimbus cells, and `CloudCellClassifier.isTornadoEligible` gates future funnel spawning.
- New config: `cloudVolumetricRendererEnabled` (pipeline master switch; legacy per-field CloudField renderer remains the automatic fallback) and `enableVolumetricGroundShadows` (post-process darkening; the shadow texture stays published when off).
