...
public class SpikeStateStorage {
    ...
    public static void saveAll(ServerLevel world) {
            ...
            for (Map.Entry<ForecastRegionId, SpikeState> entry : SpikeManager.getAllStates().entrySet()) {
                JsonObject data = new JsonObject();
                SpikeState state = entry.getValue();

                data.addProperty("rx", entry.getKey().rx());
                data.addProperty("rz", entry.getKey().rz());
                data.addProperty("dim", entry.getKey().dimension().location().toString());
                ...
                root.add(entry.getKey().rx() + "," + entry.getKey().rz() + "@" + entry.getKey().dimension().location(), data);
            }
    ...
    public static void loadAll(ServerLevel world) {
            ...
            try (Reader r = Files.newBufferedReader(SAVE_PATH)) {
                ...
                Map<ForecastRegionId, SpikeState> loaded = new HashMap<>();
                ...
                for (Map.Entry<String, JsonElement> entry : root.entrySet()) {
                    ...
                    ResourceLocation dim = ResourceLocation.parse(obj.get("dim").getAsString());
                    ForecastRegionId key = new ForecastRegionId(rx, rz,
