...
// Severe layer map 0 (RGBA16F, premultiplied endpoints):
// R = BASE support
// G = BASE top * BASE support
// B = primary CORE support
// A = primary CORE base * support
...
void main() {
    ...
    for (int i = 0; i < MAX_CELLS; i++) {
        ...
        if (role == 2) {
            if (support > baseBestSupport) {
                baseBestSupport = support;
                baseSupport = support;
                baseTop = localTop;
            }
        } else {
